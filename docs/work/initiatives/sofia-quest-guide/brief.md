...
- Sofia hint generation should use explicit current-state hint context so the
  ...
  facilitation line.
- Sofia should give a quest hint only when the player directly addresses Sofia
  ...
  Unaddressed help requests do not route to Sofia and do not advance the quest.
  Ordinary conversation, door/code comments, and vibe-coding/VCC questions use
  one general Sofia conversation route where Claude answers from the brief and
  current context.
